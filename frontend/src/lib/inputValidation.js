...
export const sanitizePaymentReferenceInput = (value) =>
  String(value ?? "").replace(/[^a-zA-Z0-9]/g, "").slice(0, INPUT_LIMITS.paymentReferenceMax);
...
export const validatePaymentReference = (value, fieldName = "Payment reference number") => {
  const normalized = sanitizePaymentReferenceInput(value).trim();
  const refRegex = new RegExp(`^[a-zA-Z0-9]{${INPUT_LIMITS.paymentReferenceMin},${INPUT_LIMITS.paymentReferenceMax}}$`);
  if (!refRegex.test(normalized)) {
    throw new Error(
      `${fieldName} must be alphanumeric and between ${INPUT_LIMITS.paymentReferenceMin} and ${INPUT_LIMITS.paymentReferenceMax} characters.`
    );
  }
  return normalized;
};
