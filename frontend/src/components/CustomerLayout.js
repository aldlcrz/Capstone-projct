...
export default function CustomerLayout({ children }) {
  ...
  const fetchNotifications = React.useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      const res = await api.get("/notifications", { params: { role: "customer" } });
      ...
      const msg = getApiErrorMessage(error, "Failed to fetch notifications");
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      storedUser = getStoredUserForRole("customer");
    } catch (error) { }
    setUser(storedUser);
    ...
    // Prevents one tab from silently 'turning into' another account logged in elsewhere.
    const checkAuth = (event) => {
      // Ignore storage events for unrelated keys
      if (event?.type === "storage" && event.key !== null && event.key !== "customer_user" && event.key !== "customer_token") return;

      const newUser = getStoredUserForRole("customer");
      const newToken = getTokenForRole("customer");

      // Case 1: Logout in another tab (newToken is null/empty)
      ...
      // Case 2: Identity Conflict (New user detected that doesn't match this tab's pinned identity)
      if (initialSession.current.token && newToken !== initialSession.current.token) {
        try {
          // If the IDs don't match, we have a hard conflict. 
          // We must reload to prevent the UI from showing User A while the API uses User B's token.
          if (String(newUser?.id) !== String(initialSession.current.userId)) {
    ...
    if (socket && storedUser?.id) {
      socket.on('new_notification', (incoming) => {
        ...
        if (incoming.targetRole === 'customer') {
    ...
    <div data-panel="customer" className="flex h-screen bg-[#F7F3EE] overflow-hidden">
      ...
      >
        <div className="p-10 flex flex-col h-full">
          <div className="mb-12">
            <Link href="/home" className="font-serif text-lg font-bold text-[var(--charcoal)] tracking-tighter">
              LUMBARONG
            </Link>
      ...
      <div className="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        <header className="sticky top-0 z-40 flex min-h-[72px] items-center border-b border-[var(--border)] bg-white py-3 shadow-sm lg:h-[72px] lg:py-0">
          <div className="fluid-container flex min-w-0 items-center justify-between gap-3 px-4 lg:px-10">
            ...
            <div className="flex min-w-0 items-center lg:hidden">
              <Link href="/home" className="font-serif text-sm font-bold tracking-tighter text-[var(--charcoal)]">
                LUMBARONG
              </Link>
            </div>
            ...
            <div className="flex shrink-0 items-center gap-3 sm:gap-4 md:gap-6">
              ...
              <Link href="/profile" className="lg:hidden shrink-0 w-10 h-10 rounded-xl bg-[var(--sand)] border-2 border-white shadow-md flex items-center justify-center text-white font-serif text-base font-bold uppercase overflow-hidden transition-transform active:scale-95">
                {user?.profilePhoto ? (
                  <img
              ...
              <button 
                onClick={handleLogout}
                className="lg:hidden w-10 h-10 flex items-center justify-center bg-red-50 rounded-xl text-red-600 hover:bg-red-100 transition-colors"
                aria-label="Logout"
              >
        ...
        {/* Scrollable Content */}
        <main className="fluid-container !pt-6 flex-1 overflow-y-auto pb-[145px] sm:!pt-8 sm:pb-[155px] lg:!pt-10 lg:pb-10">
