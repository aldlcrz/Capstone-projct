...
export default function EditProductClient({ id: propId }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [prodRes, catsData] = await Promise.all([
          api.get(`/products/${id}`),
          fetchCategories()
        ]);
  ...
  if (fetching) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--muted)]">Loading product...</p>
    </div>
  );
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <h1 className="font-serif text-lg lg:text-xl font-bold tracking-tight text-[var(--charcoal)]">
            Edit <span className="text-[var(--rust)] italic lowercase">product</span>
          </h1>
        </div>
      </div>
