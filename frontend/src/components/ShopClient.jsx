...
export default function ShopClient() {
  ...
  useEffect(() => {
    try {
      ...
      if (stored && stored.following && stored.following.includes(userIdStr)) {
        setIsFollowing(true);
      }
    } catch (e) {
  ...
  const handleFollow = async () => {
    if (userRole !== 'customer' && userRole !== 'admin' && userRole !== 'seller') {
      router.push("/login"); // User must be logged in to follow
      return;
    }
    try {
      const res = await api.post(`/users/seller/${id}/follow`);
  ...
  const displayedProducts = React.useMemo(() => {
    let p = [...products];
    if (activeTab === 'sale') {
      p = p.filter(prod => false); // no On Sale products yet
    } else if (activeTab === 'rated') {
      p = p.sort((a, b) => (Number(b.rating) || 5) - (Number(a.rating) || 5));
    }
    return p;
  }, [products, activeTab]);
                ...
                {seller.facebookLink && (
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex justify-between w-full pr-4">
                      ...
                      <a href={seller.facebookLink.startsWith('http') ? seller.facebookLink : `https://${seller.facebookLink}`} target="_blank" rel="noreferrer" className="text-[#1877F2] font-medium font-serif hover:underline truncate max-w-[100px] text-right">View Profile</a>
                    </div>
                  </div>
                )}
                {seller.instagramLink && (
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex justify-between w-full pr-4">
                      ...
                      <a href={seller.instagramLink.startsWith('http') ? seller.instagramLink : `https://${seller.instagramLink}`} target="_blank" rel="noreferrer" className="text-[#E4405F] font-medium font-serif hover:underline truncate max-w-[100px] text-right">View Page</a>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="max-w-6xl mx-auto px-4 lg:px-0 py-10">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-4 text-sm font-medium">
              <button onClick={() => setActiveTab('all')} className={`${activeTab==='all' ? 'text-[var(--rust)] border-b-2 border-[var(--rust)]' : 'text-gray-500 hover:text-[var(--rust)] transition-colors'} pb-1`}>All Products</button>
              <button onClick={() => setActiveTab('sale')} className={`${activeTab==='sale' ? 'text-[var(--rust)] border-b-2 border-[var(--rust)]' : 'text-gray-500 hover:text-[var(--rust)] transition-colors'} pb-1`}>On Sale</button>
              <button onClick={() => setActiveTab('rated')} className={`${activeTab==='rated' ? 'text-[var(--rust)] border-b-2 border-[var(--rust)]' : 'text-gray-500 hover:text-[var(--rust)] transition-colors'} pb-1`}>Highest Rated</button>
            </div>
          </div>
