  ...
  useEffect(() => {
    ...
    const fetchCustomerData = async () => {
      try {
        ...
        const customerOrders = (Array.isArray(rawOrders) ? rawOrders : []).filter(order => order.customer && String(order.customer.id) === String(id));
    ...
    <SellerLayout>
      <div className="space-y-10 mb-20 animate-fade-in">
        <button onClick={() => router.back()} className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-[var(--muted)] hover:text-[var(--rust)] transition-colors group">
          <div className="w-10 h-10 rounded-full bg-white border border-(--border) flex items-center justify-center shadow-sm group-hover:-translate-x-1 transition-transform">
        ...
        <div className="artisan-card py-24 text-center space-y-4 shadow-sm">
           ...
           <p className="text-[var(--muted)] max-w-sm mx-auto">We could not locate any orders linked to this customer in your workshop registry.</p>
        </div>
      </div>
    </SellerLayout>
  );
          ...
          <div className="xl:col-span-4 space-y-8">
            <div className="artisan-card p-10 space-y-8 shadow-xl relative overflow-hidden bg-gradient-to-br from-white to-[var(--cream)]/30 border-none">
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-(--border)">
                 <div className="bg-white p-4 rounded-2xl shadow-sm border border-(--border) text-center">
              ...
              <div className="space-y-4 pt-4">
                ...
                <div className="flex items-start gap-4 bg-white p-4 rounded-2xl border border-(--border) shadow-sm">
            ...
            <div className="space-y-6">
              {orders.map((order, idx) => (
                <motion.div 
                  ...
                  className="artisan-card p-8 flex flex-col md:flex-row gap-8 shadow-md border-(--border) hover:border-[var(--rust)]/30 transition-all bg-white"
                >
                    ...
                    <div className="space-y-4">
                      {(order.items ? (typeof order.items === 'string' ? JSON.parse(order.items) : order.items) : []).map((item, i) => (
                        <div key={i} className="flex gap-4">
                          <div className="w-16 h-20 bg-[var(--cream)] rounded-xl relative overflow-hidden border border-(--border) shrink-0">
                             <Image src={item.image?.[0] || item.image || "/images/placeholder.png"} alt={item.name} fill sizes="64px" className="object-cover" unoptimized />
                          </div>
                          <div>
                  ...
                  <div className="w-full md:w-64 shrink-0 bg-[var(--input-bg)] p-6 rounded-2xl flex flex-col justify-between border border-(--border)">
