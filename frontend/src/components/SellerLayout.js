  ...
  return (
    <div data-panel="seller" className="flex h-screen bg-[var(--cream)] overflow-hidden">
      ...
      <motion.aside
        initial={{ x: -280 }}
        animate={{ x: 0 }}
        className="hidden lg:flex flex-col w-[280px] h-full bg-white border-r border-(--border) overflow-hidden"
      >
        <div className="p-10 flex flex-col h-full">
          ...
          <div className="mt-10 pt-8 border-t border-(--border) shrink-0">
            <button
      ...
      <div className="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        {/* Top Sticky Header */}
        <header className="sticky top-0 z-40 bg-white border-b border-(--border) h-[72px] flex items-center shrink-0">
          <div className="fluid-container flex items-center justify-between px-4 lg:px-10">
            ...
            <div className="flex items-center gap-2 md:gap-6">
                  ...
                  {isPopoutOpen && (
                    <>
                      ...
                      <motion.div
                        ...
                        className="absolute right-0 mt-3 w-80 md:w-96 max-h-[500px] bg-white rounded-2xl shadow-2xl border border-(--border) overflow-hidden z-50 flex flex-col"
                      >
                        <div className="p-5 border-b border-(--border) flex items-center justify-between bg-stone-50/50">
                          ...
                          {notifications.filter(n => !dismissedNotificationIds.includes(n.id)).length > 0 && (
                            <button
                              ...
                              className="text-[10px] bg-[var(--cream)] border border-(--border) hover:bg-[var(--rust)] hover:text-white transition-all rounded-md px-2 py-1 font-bold text-[var(--charcoal)] tracking-wider flex items-center gap-1"
                            >
              ...
              <Link href="/seller/profile" className="hidden lg:flex items-center gap-3 group">
                ...
                <div className="relative w-10 h-10 rounded-xl bg-[var(--bark)] border-2 border-white shadow-md flex items-center justify-center text-white font-serif text-lg font-bold overflow-hidden transition-transform active:scale-95">
                  {user?.profilePhoto ? (
                    <Image src={user.profilePhoto} alt={user.name} fill sizes="40px" className="object-cover" unoptimized />
                  ) : (
              ...
              <Link href="/seller/profile" className="relative lg:hidden shrink-0 w-10 h-10 rounded-xl bg-[var(--bark)] border-2 border-white shadow-md flex items-center justify-center text-white font-serif text-base font-bold overflow-hidden transition-transform active:scale-95">
                {user?.profilePhoto ? (
                  <Image src={user.profilePhoto} alt={user.name} fill sizes="40px" className="object-cover" unoptimized />
                ) : (
                  user?.name ? user.name[0] : "A"
