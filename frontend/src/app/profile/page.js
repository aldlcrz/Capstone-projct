...
import { api, getStoredUserForRole, setStoredUserForRole } from "@/lib/api";
...
export default function CustomerProfile() {
   ...
   const fetchProfile = useCallback(async (isBackground = false) => {
      try {
         ...
         setStoredUserForRole("customer", userData);
      } catch (error) {
         console.error("Failed to fetch fresh profile", error);
         // If fetch fails, we still have the local storage as fallback
         const storedUser = getStoredUserForRole("customer") || {};
         setUser(storedUser);
         setEditForm({
               ...
               <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="lg:col-span-8">
                     <div className="bg-white rounded-[2rem] shadow-sm border border-[var(--border)] overflow-hidden min-h-[500px]">
                        ...
                        {/* Premium Tabs Header */}
                        <div className="flex bg-[#FBF8F4] px-2 pt-2 border-b border-[var(--border)] overflow-x-auto no-scrollbar">
                           {["Account Info", "Change Password", "Address Book"].map((tab) => {
                              const isActive = activeTab === tab;
                              return (
                                 <button
                                    key={tab}
                                    onClick={() => setActiveTab(tab)}
                                    className={`px-8 py-5 text-[10px] font-bold uppercase tracking-[0.16em] transition-all relative group flex-shrink-0 ${
                                       isActive ? "text-[var(--rust)]" : "text-[var(--muted)] hover:text-[var(--charcoal)]"
                                    }`}
                                 >
                                    ...
                                    {isActive && (
                                       <motion.div 
                                          ...
                                          className="absolute bottom-0 left-0 right-0 h-[3px] bg-[var(--rust)] rounded-t-full shadow-[0_-2px_6px_rgba(192,66,42,0.2)]" 
                                       />
                                    )}
                                    {!isActive && (
                                       <div className="absolute inset-0 bg-transparent group-hover:bg-[#F0EBE5]/50 transition-colors rounded-t-xl" />
                                    )}
                                 </button>
                              );
                           })}
                              ...
                              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 max-w-xl">
                                 ...
                                 <div className="space-y-6">
                                    ...
                                    <button 
                                       ...
                                       className="w-full bg-gradient-to-r from-[#C0420A] via-[#D35400] to-[#E8604A] text-white py-4 rounded-xl text-[10px] font-extrabold uppercase tracking-[0.15em] flex items-center justify-center gap-2 hover:shadow-[0_8px_20px_rgba(192,66,42,0.3)] hover:-translate-y-0.5 transition-all mt-2 disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
