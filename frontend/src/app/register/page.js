...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.role === "seller") {
      ...
      const cleanMobile = sellerData.mobileNumber.replace(/\D/g, "");
      const cleanGcash = sellerData.gcashNumber.replace(/\D/g, "");
      ...
      if (!phoneRegex.test(cleanGcash)) {
        setError("Please enter a valid GCash number (e.g., 09123456789).");
        setLoading(false);
        return;
      }
      ...
      data.append("mobileNumber", sellerData.mobileNumber);
      data.append("gcashNumber", sellerData.gcashNumber);
