  ...
  return (
    <CustomerLayout>
      <div className="max-w-[900px] mx-auto space-y-8 mb-20 px-4 md:px-0">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 pb-2">
          <div className="space-y-4">
             <div className="flex items-center gap-3">
               <span className="w-8 h-0.5 bg-[#B94232]"></span>
               <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#B94232]">NOTIFICATIONS</span>
             </div>
             <h1 className="font-serif text-[42px] font-bold tracking-tight text-[#2A2A2A] leading-none">
               Alerts & Updates
             </h1>
             <p className="text-sm font-bold text-[#B94232]">{unreadCount} unread notification{unreadCount !== 1 ? 's' : ''}</p>
          </div>
          <button 
           ...
           <AnimatePresence initial={false}>
             {loading ? (
                <div className="p-24 text-center space-y-4">
                   ...
                   <p className="text-sm text-[var(--muted)] italic">Loading your latest updates...</p>
                </div>
             ) : notifications.length === 0 ? (
                <div className="p-24 text-center space-y-4">
                   <Bell className="w-12 h-12 text-[var(--muted)] mx-auto opacity-20" />
                   <h3 className="font-serif text-xl font-bold text-[#2A2A2A] opacity-80">You're all caught up!</h3>
                   <p className="text-sm text-[var(--muted)] italic">No new alerts or updates at this time.</p>
                </div>
             ) : (
                    ...
                    <div className="flex-1 w-full space-y-3 pt-0.5">
                      ...
                      <div className="flex justify-between items-start">
                         ...
                         <div className="flex items-center gap-3">
                            ...
                            {!notif.read && (
                               <span className="text-[9px] font-bold text-[#B94232] border border-[#F4A8A3] bg-[#FEF2F1] px-2 py-0.5 rounded-md uppercase tracking-wider">NEW</span>
                            )}
                         </div>
                      </div>
                      ...
                      <div className="space-y-1.5">
                         <h3 className="text-lg font-bold text-[#2A2A2A]">{notif.title}</h3>
                         <div className="flex">
                      ...
                      <div className="pt-2 flex items-center gap-6">
                         <button onClick={() => handleNotificationClick(notif)} className="flex items-center gap-1 text-[11px] font-extrabold text-[var(--muted)] hover:text-[#2A2A2A] uppercase tracking-[0.1em] transition-colors group/view">
                            VIEW DETAILS <ArrowRight className="w-3.5 h-3.5 group-hover/view:translate-x-1 transition-transform ml-1" />
                         </button>
        ...
        <div className="bg-[#1A1613] text-[#EFEAE2] p-8 md:p-12 rounded-[2rem] flex flex-col md:flex-row items-center justify-between gap-10 mt-6 shadow-2xl">
           <div className="space-y-3 flex-1">
              <h3 className="font-serif text-3xl font-bold tracking-tight italic">Notification Settings</h3>
              <p className="text-sm text-[#EFEAE2]/60 font-medium italic max-w-sm leading-relaxed">Choose how you want to receive updates about your orders, messages, and account.</p>
           </div>
           
           <div className="flex flex-col sm:flex-row md:flex-col gap-4 w-full md:w-auto shrink-0">
              <button className="w-full md:w-56 py-3.5 md:py-4 bg-[#B94232] hover:bg-[#A3392A] text-white rounded-[1rem] sm:rounded-xl text-[10px] font-extrabold uppercase tracking-[0.15em] flex items-center justify-center gap-2.5 transition-colors shadow-lg border-b-2 border-transparent">
                 <Bell className="w-4 h-4" /> TURN ON PUSH
              </button>
              <button className="w-full md:w-56 py-3.5 md:py-4 bg-transparent hover:bg-[#2F2721] text-[#EFEAE2] border border-[#3C3229] rounded-[1rem] sm:rounded-xl text-[10px] font-extrabold uppercase tracking-[0.15em] flex items-center justify-center gap-2.5 transition-colors">
                 <Mail className="w-4 h-4" /> MANAGE EMAIL
              </button>
           </div>
        </div>
