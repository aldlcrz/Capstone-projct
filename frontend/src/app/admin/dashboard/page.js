...
export default function AdminDashboard() {
  ...
  return (
    <AdminLayout>
      <div className="space-y-6 mb-16 sm:mb-20 animate-fade-down">
        {/* Page Heading */}
        <div className="flex flex-col gap-4 md:gap-6">
          <div>
            <div className="eyebrow">Enterprise Overview</div>
            <h1 className="font-serif text-charcoal text-xl font-bold tracking-tight">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-3">
            {/* Filter Tabs */}
            <div className="flex items-center gap-1 p-1 bg-white border border-(--border) rounded-xl shadow-sm flex-wrap sm:flex-nowrap">
            ...
            <div className="flex items-center gap-2 ml-auto sm:ml-0">
              <button
                onClick={refresh}
                disabled={refreshing}
                className="p-2.5 bg-white border border-(--border) rounded-xl hover:border-[var(--rust)] hover:text-[var(--rust)] transition-all shadow-sm flex-shrink-0"
                title="Refresh data"
              >
        ...
        <div className="space-y-6">
          ...
          <div className="artisan-card flex flex-col justify-start">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 pb-2 gap-2">
              <div>
                <h3 className="text-base sm:text-lg font-bold text-[var(--charcoal)] mb-1">System Sales</h3>
                <div className="text-xs text-[var(--muted)] tracking-wider capitalize">Total earnings • {dateFilter} view</div>
              </div>
            </div>

            <div className="h-[120px] lg:h-[130px] mt-4">
              <ResponsiveContainer width="100%" height="100%" key={`revenue-${dateFilter}`}>
                <AreaChart data={analytics.revenueSeries} margin={{ left: 5, right: 10, top: 10, bottom: 0 }}>
                  <defs>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
          ...
          <div className="artisan-card flex flex-col justify-start">
            ...
            <div className="h-[120px] lg:h-[130px] mt-4">
              <ResponsiveContainer width="100%" height="100%" key={`growth-${dateFilter}`}>
                <AreaChart data={analytics.monthlySignups} margin={{ left: 0, right: 10, top: 10, bottom: 0 }}>
                  <defs>
                  ...
                  <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#8C7B70", fontWeight: "600" }} interval="preserveStartEnd" minTickGap={30} dy={10} />
                  <YAxis
                    axisLine={false}
                    tickLine={false}
                    width={30}
          ...
          {/* Top Locations */}
          <div className="artisan-card min-h-[220px] flex flex-col">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 my-6 lg:my-8">
          ...
          <div className="artisan-card flex flex-col min-h-[250px]">
            ...
            {!analytics.topProducts || analytics.topProducts.length === 0 ? (
              ...
              <div className="flex-1 flex flex-col space-y-2">
                <div className="grid grid-cols-12 gap-2 px-2 pb-2 text-[8px] sm:text-[9px] uppercase font-bold tracking-widest text-[var(--muted)] border-b border-(--border)">
                ...
                {analytics.topProducts.slice(0, 5).map((prod, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="grid grid-cols-12 gap-2 items-center bg-[#f7f6f2] rounded-xl p-2 hover:bg-[#FAF9F7] transition-all"
                    >
          ...
          <div className="artisan-card flex flex-col min-h-[250px]">
            ...
            {!analytics.topCategories || analytics.topCategories.length === 0 ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center">
                <div className="w-full h-[140px] sm:h-[150px]">
                    ...
                    <PieChart>
                      ...
                      <Tooltip
                        ...
                        formatter={(val, name, props) => {
                          const total = analytics.topCategories.reduce((sum, entry) => sum + entry.value, 0);
                          const pct = total > 0 ? Math.round((val / total) * 100) : 0;
                          return [`${val} items (${pct}%)`, "Sales"];
                        }}
                      />
                    </PieChart>
                  </ResponsiveContainer>
                ...
                {/* Custom Legend */}
                <div className="w-full mt-3 sm:mt-4 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 px-2 sm:px-4">
                  {(() => {
                    const total = analytics.topCategories.reduce((sum, entry) => sum + entry.value, 0);
                    return analytics.topCategories.map((entry, index) => {
                      ...
                      return (
                        <div key={index} className="flex items-center gap-1.5">
                          ...
                          <span className="text-[9px] sm:text-[11px] text-[var(--muted)] font-bold">
                            {percentage}% <span className="opacity-40 font-medium">({entry.value})</span>
                          </span>
                        </div>
                      );
                    });
  ...
  return (
    <motion.div whileHover={{ y: -4 }} className="artisan-card group cursor-pointer relative overflow-hidden">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${colorMap[color]}`}>
          {icon}
        </div>
        <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" title="Live" />
      </div>
      {loading ? (
        <div className="h-5 w-16 bg-[var(--cream)] animate-pulse rounded-lg mb-1" />
      ) : (
        <div className="text-lg font-serif font-bold text-[var(--charcoal)] group-hover:text-[var(--rust)] transition-colors mb-1">
          {value}
        </div>
      )}
      <div className="text-[9px] font-bold text-[var(--muted)] opacity-60 uppercase tracking-widest">
        {label}
      </div>
    </motion.div>
  );
