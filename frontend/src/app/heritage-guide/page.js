...
function StepSection({ number, title, subtitle, description, icon, image, reversed }) {
  return (
    ...
    >
      ...
      {/* Image side */}
      <div className="lg:w-1/2 w-full">
        <div className="relative aspect-[4/3] rounded-[3rem] overflow-hidden shadow-2xl grayscale hover:grayscale-0 transition-all duration-1000">
          <Image src={image} className="object-cover" alt={title} fill sizes="(min-width: 1024px) 50vw, 100vw" unoptimized />
        </div>
      </div>
    </motion.div>
  );
