...
export default function AddProductPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.tags.length === 0) {
      showToast("error", "Please select at least one audience tag (Men, Women, or Kids).");
      return;
    }
    ...
    if (formData.allowGcash) {
      if (!formData.gcashNumber || !formData.gcashNumber.trim()) {
        showToast("error", "GCash Mobile Number is required when GCash is enabled.");
        return;
      }
      if (!formData.gcashName || !formData.gcashName.trim()) {
        showToast("error", "GCash Account Name is required when GCash is enabled.");
        return;
      }
      if (!gcashQrFile && !gcashQrPreview) {
        showToast("error", "GCash QR Code image is required when GCash is enabled.");
        return;
      }
    }
    if (formData.allowMaya) {
      if (!formData.mayaNumber || !formData.mayaNumber.trim()) {
        showToast("error", "Maya Mobile Number is required when Maya is enabled.");
        return;
      }
      if (!formData.mayaName || !formData.mayaName.trim()) {
        showToast("error", "Maya Account Name is required when Maya is enabled.");
        return;
      }
      if (!mayaQrFile && !mayaQrPreview) {
        showToast("error", "Maya QR Code image is required when Maya is enabled.");
        return;
      }
    }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="font-serif text-lg lg:text-xl font-bold tracking-tight text-(--charcoal)">
              Create New <span className="text-(--rust) italic lowercase">product</span>
            </h1>
          </div>
        </div>
