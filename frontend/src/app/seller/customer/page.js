...
import { api, clearSession, getTokenForRole } from "@/lib/api";
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="relative bg-white w-full max-w-2xl max-h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-(--border)"
      >
        {/* Header */}
        <div className="p-6 border-b border-(--border) flex items-center justify-between bg-[var(--cream)]">
          ...
          <button onClick={onClose} className="p-2.5 hover:bg-white rounded-xl transition-colors border border-(--border) group">
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(order.items || order.orderItems || []).slice(0, 3).map((item, i) => (
                      <div key={i} className="flex items-center gap-1 bg-[var(--cream)] border border-(--border) rounded-md px-2 py-1">
        ...
        {/* Footer */}
        <div className="p-5 border-t border-(--border) bg-[var(--cream)]/30 flex items-center justify-between">
...
function MoreMenu({ customer, onMessage, onViewPortfolio, onReport }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen(v => !v)}
        className="p-2.5 bg-white border border-(--border) rounded-xl text-[var(--muted)] hover:text-[var(--rust)] hover:bg-[var(--cream)] shadow-sm transition-all active:scale-95"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-12 z-50 bg-white border border-(--border) rounded-2xl shadow-2xl overflow-hidden w-56"
          >
            <button
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-[var(--charcoal)] hover:bg-[var(--cream)] hover:text-[var(--rust)] transition-all border-t border-(--border) text-left"
            >
              <ChevronRight className="w-4 h-4" /> View Portfolio
            </button>
            <button
              onClick={() => { onReport(); setOpen(false); }}
              className="w-full flex items-center gap-3 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-red-500 hover:bg-red-50 transition-all border-t border-(--border) text-left"
            >
...
function SellerCustomersContent() {
  ...
  }, [loading, customers, targetId, selectedPortfolio]);
      ...
      <div className="space-y-8 mb-20 pt-2 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex bg-white rounded-2xl border border-(--border) p-1.5 shadow-sm overflow-hidden">
              <button
          ...
          {loading ? (
            ...
            filtered.map((customer, idx) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex flex-1 items-center gap-4 lg:gap-8 w-full border-t lg:border-none border-(--border) pt-4 lg:pt-0">
                ...
                {/* Action Buttons */}
                <div className="flex items-center justify-between lg:justify-end gap-3 w-full lg:w-auto pt-4 lg:pt-0 border-t lg:border-none border-(--border)">
