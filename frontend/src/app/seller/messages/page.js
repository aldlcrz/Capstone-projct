...
function SellerMessages() {
  ...
  useEffect(() => {
    ...
    init();
  }, [customerIdParam, customerNameParam]);
  ...
  return (
    <SellerLayout>
      <div className="flex overflow-hidden bg-white lg:h-[calc(100vh-140px)] lg:rounded-2xl lg:shadow-xl lg:border lg:border-(--border) max-lg:fixed max-lg:top-[72px] max-lg:left-0 max-lg:right-0 max-lg:bottom-[112px] z-10">

        {/* ── Left: Thread List ── */}
        <aside className={`${activeThread ? "hidden lg:flex" : "flex"} flex-col w-full lg:w-72 xl:w-80 border-r border-(--border) bg-[#FAFAF9] shrink-0`}>
          <div className="px-4 pt-4 pb-3 border-b border-(--border)">
            ...
            <div className="relative">
              ...
              <input type="text" value={threadSearch} onChange={e => setThreadSearch(e.target.value)} placeholder="Filter inquiries..." className="w-full pl-7 pr-3 py-1.5 bg-white border border-(--border) rounded-lg text-[10px] outline-none focus:border-[var(--rust)] transition-all" />
            </div>
          </div>
            ...
            <>
              ...
              <div className="px-4 py-3 border-b border-(--border) flex items-center justify-between shrink-0">
                ...
                <div className="relative shrink-0" ref={menuRef}>
                  ...
                  {menuOpen && (
                    <div className="absolute right-0 top-[calc(100%+6px)] z-50 w-52 bg-white rounded-xl shadow-2xl border border-(--border) overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-[var(--cream)]/20 custom-scrollbar min-h-0">
                {messages.length === 0 && (
                  <div className="text-center py-6"><span className="bg-white px-4 py-1.5 rounded-full border border-(--border) text-[9px] font-bold uppercase tracking-widest text-[var(--muted)] shadow-sm">{activeThread.isGhost ? "Send your first message" : "Heritage Thread Active"}</span></div>
                )}
                {messages.map((msg, i) => {
                  ...
                  return (
                    <motion.div key={i} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className={`flex items-end gap-2 ${isMine ? "justify-end" : "justify-start"}`}>
                      ...
                      <div className={`max-w-[70%] px-3 py-2 rounded-xl text-[11px] leading-relaxed ${isMine ? "bg-[var(--rust)] text-white rounded-br-sm" : "bg-white text-[var(--charcoal)] border border-(--border) rounded-bl-sm shadow-sm"}`}>
                        {msg.content}
              ...
              <div className="border-t border-(--border) bg-white shrink-0">
                <AnimatePresence>
                  {showQuickReplies && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18 }} className="overflow-hidden border-b border-(--border)">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {QUICK_REPLIES.map(qr => (
                            <button key={qr.label} type="button" onClick={() => { setNewMessage(qr.text); setShowQuickReplies(false); setTimeout(() => textareaRef.current?.focus(), 50); }}
                              className="px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider border border-(--border) rounded-full bg-[var(--cream)] hover:bg-[var(--rust)] hover:text-white hover:border-[var(--rust)] transition-all active:scale-95">{qr.label}</button>
                          ))}
                        </div>
                      </div>
                    </motion.div>
                ...
                <form onSubmit={sendMessage} className="px-4 py-3">
                  <div className="flex items-center gap-2 bg-[var(--cream)]/40 border border-(--border) rounded-xl px-3 py-1.5 focus-within:border-[var(--rust)] focus-within:bg-white transition-all">
