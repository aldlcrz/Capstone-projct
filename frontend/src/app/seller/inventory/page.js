      ...
      <div className="space-y-8 mb-20 pt-2">
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-end gap-6 mt-4 animate-fade-up">
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-white border border-(--border) rounded-xl text-xs font-bold uppercase tracking-widest text-[var(--muted)] hover:text-[var(--rust)] hover:border-[var(--rust)] transition-all w-48 justify-center"
          >
            ...
            {products
              ...
              .map((product, i) => {
                ...
                return (
                  ...
                  >
                    <div className={`relative shrink-0 overflow-hidden bg-[var(--cream)] rounded-xl border border-(--border) ${view === "list" ? 'w-20 h-20' : 'w-full h-64 border-none border-b rounded-none'}`}>
                      <Link href={`/products?id=${product.id}&preview=1`} className="block w-full h-full">
                        <Image
                          src={getProductImageSrc(product.image)}
                          className="object-cover group-hover:scale-110 transition-transform duration-1000 cursor-pointer"
                          alt={product.name}
                          fill
                          sizes={view === "list" ? "80px" : "(min-width: 1024px) 33vw, (min-width: 768px) 50vw, 100vw"}
                          unoptimized
                    ...
                    <div className={`flex-1 ${view === "list" ? 'grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6 items-center' : 'p-6 space-y-6 flex flex-col justify-between h-full'}`}>
                      ...
                      <div className="flex items-center justify-between md:justify-end gap-3 pt-4 md:pt-0 border-t md:border-none border-(--border)">
