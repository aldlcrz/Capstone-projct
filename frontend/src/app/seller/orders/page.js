...
export default function SellerOrders() {
  ...
  }, [socket, fetchOrders, fetchRefunds]);
  ...
  return (
    <SellerLayout>
      <div className="space-y-8 animate-fade-in pt-2 mb-20 px-4 sm:px-6 lg:px-8 max-w-[1600px] mx-auto">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="relative group">
              ...
              <input
                ...
                className="bg-white border border-(--border) rounded-2xl pl-12 pr-6 py-4 text-sm w-full md:w-[320px] outline-none focus:border-[var(--rust)] focus:ring-4 focus:ring-[var(--rust)]/5 transition-all shadow-sm"
              />
            </div>
          </div>
        </header>
        ...
        <div className="bg-white rounded-[2.5rem] border border-(--border) overflow-hidden shadow-xl shadow-stone-200/50">
          {/* Desktop Table */}
          <div className="hidden lg:block overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[1000px]">
              <thead>
                <tr className="bg-[var(--stone)]/30 border-b border-(--border)">
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 lg:p-6">
      ...
      <motion.div
        ...
        className="relative bg-white w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-2xl sm:rounded-3xl shadow-2xl flex flex-col border border-(--border)"
      >
        {/* Modal Header */}
        <div className="p-4 sm:p-6 border-b border-(--border) flex items-start sm:items-center justify-between bg-[var(--warm-white)] text-[var(--charcoal)] relative overflow-hidden shrink-0">
          ...
          <button onClick={onClose} className="absolute top-3 right-3 sm:top-5 sm:right-5 p-2 sm:p-2.5 bg-white hover:bg-[var(--cream)] rounded-full transition-all border border-(--border) group active:scale-90 z-20">
              ...
              <section>
                ...
                <div className="artisan-card space-y-4 !p-4 sm:!p-5">
                  ...
                  <div className="pt-4 border-t border-(--border)/50">
                    ...
                    <div className="bg-[var(--cream)]/50 p-3 sm:p-4 rounded-xl border border-(--border)/30">
              ...
              <div className="artisan-card flex-1 flex flex-col min-h-full !p-4 sm:!p-5">
                <div className="flex-1 space-y-3 sm:space-y-4 mb-5 sm:mb-6">
                  {(order.items || order.orderItems || []).map((item, i) => (
                    <div key={i} className="flex gap-3 sm:gap-4 p-2 sm:p-3 hover:bg-[var(--cream)]/30 rounded-xl transition-all border border-transparent hover:border-(--border) group">
                      <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-lg overflow-hidden bg-white border border-(--border) shadow-sm shrink-0">
                        <Image
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-center">
                        ...
                        {item.variant && (
                          <div className="mt-1 inline-block self-start px-1.5 py-0.5 bg-white rounded text-[7px] font-black text-[var(--muted)] border border-(--border) uppercase tracking-widest">
                            {item.variant}
                          </div>
                        )}
                      </div>
...
function MobileOrderCard({ order, index, onView, onUpdateStatus, isUpdating }) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-[var(--cream)]/60 border border-(--border)/50 rounded-2xl p-3 flex items-center gap-3">
        {/* Avatar */}
        ...
        <button
          onClick={onView}
          className="p-2 bg-white border border-(--border) text-[var(--muted)] hover:text-[var(--rust)] hover:border-[var(--rust)] rounded-xl transition-all flex-shrink-0 shadow-sm"
        >
    ...
    >
      ...
      <td className="px-8 py-6 align-top">
        <div className="flex items-center gap-3 mb-2">
          <div className="relative w-8 h-8 rounded bg-stone-100 border border-stone-200 overflow-hidden shrink-0 p-1">
            <Image src={getProductImageSrc(refund.OrderItem?.product?.image)} alt="" fill sizes="32px" className="object-cover" unoptimized />
          </div>
          <div className="min-w-0">
  ...
  return (
    ...
    >
      ...
      <div className="bg-stone-50 rounded-2xl p-3 border border-stone-100 flex items-center gap-3">
        <div className="relative w-10 h-10 rounded-lg bg-white border border-stone-200 p-1 shrink-0">
          <Image src={getProductImageSrc(refund.OrderItem?.product?.image)} alt="" fill sizes="40px" className="object-cover" unoptimized />
        </div>
        <div className="min-w-0">
