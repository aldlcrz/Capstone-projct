...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (checkoutMode === 'buy_now') {
      ...
      items = getCustomerScopedJson("checkout_items", []);
    }
  ...
  useEffect(() => {
    if (cartItems.length > 0) {
      const isGcashDisabled = firstItem.allowGcash === false || firstItem.allowGcash === 0 || firstItem.allowGcash === "false" || firstItem.allowGcash === "0";
      const isMayaDisabled = firstItem.allowMaya === false || firstItem.allowMaya === 0 || firstItem.allowMaya === "false" || firstItem.allowMaya === "0";

      if (isGcashDisabled && paymentMethod === "GCash" && !isMayaDisabled) {
        setPaymentMethod("Maya");
      } else if (isMayaDisabled && paymentMethod === "Maya" && !isGcashDisabled) {
        setPaymentMethod("GCash");
      }
    }
  }, [cartItems, paymentMethod]);

  // Always fetch fresh payment data from backend to ensure QR codes are up-to-date
  useEffect(() => {
    ...
    api.get(`/products/${productId}`)
      .then(res => {
        const prod = res.data;
        // Enrich all cart items with fresh payment data from the backend
        setCartItems(prev => prev.map(item => ({
          ...item,
          gcashNumber: prod.gcashNumber ?? item.gcashNumber,
          gcashQrCode: prod.gcashQrCode ?? item.gcashQrCode,
          mayaNumber: prod.mayaNumber ?? item.mayaNumber,
          mayaQrCode: prod.mayaQrCode ?? item.mayaQrCode,
          allowGcash: prod.allowGcash ?? item.allowGcash,
          allowMaya: prod.allowMaya ?? item.allowMaya,
          seller: prod.seller ?? item.seller,
        })));
      })
  ...
  const sellerGcashName = currentSeller?.name || firstItem?.artisan || "Artisan Workshop";
  ...
  const sellerMayaName = currentSeller?.name || firstItem?.artisan || "Artisan Workshop";
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 1) {
      ...
      if (isGcashPayment || isMayaPayment) {
        ...
        const screenshotError = await validateImageFile(screenshot, "Payment proof");
        if (screenshotError) {
  ...
  const handlePlaceOrder = async (validatedAddress = null, validatedPaymentReference = "") => {
    ...
    try {
      ...
      alert(getApiErrorMessage(err, "Failed to transmit order to registry."));
      setLoading(false);
    }
  };
      ...
      <div className={`mx-auto animate-fade-in relative z-10 ${isBuyNowMode ? 'max-w-6xl space-y-8 mb-12' : 'max-w-7xl space-y-8 sm:space-y-10 lg:space-y-12 mb-16 lg:mb-20'}`}>
        ...
        <div className={`flex flex-col border-b border-[var(--border)] ${isBuyNowMode ? 'gap-6 pb-8 lg:flex-row lg:items-center lg:justify-between' : 'gap-6 pb-8 lg:flex-row lg:items-end lg:justify-between lg:pb-12'}`}>
          <div className="max-w-2xl">
            ...
            <h1 className={`font-serif font-bold tracking-tighter text-[var(--charcoal)] uppercase ${isBuyNowMode ? 'text-lg sm:text-xl' : 'text-xl sm:text-2xl'}`}>
              <span className="text-[var(--rust)] italic lowercase">checkout</span>
            </h1>
            {!isBuyNowMode && (
              <p className="text-[var(--muted)] leading-relaxed text-sm sm:text-base mt-4 max-w-2xl">
            ...
            >
                ...
                <AnimatePresence mode="wait">
                  {currentStep === 1 && (
                    ...
                    >
                      <div className="mb-2 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <h3 className={`font-serif font-bold text-[var(--charcoal)] tracking-tight flex items-center gap-4 ${isBuyNowMode ? 'text-lg sm:text-xl' : 'text-lg sm:text-xl lg:text-2xl'}`}>
                  ...
                  {currentStep === 2 && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, x: 0 }}
                      exit={{ opacity: 0, x: 20 }}
                      className={`artisan-card shadow-lg bg-white/80 backdrop-blur-xl ${isBuyNowMode ? 'p-5 sm:p-6 space-y-5 sm:space-y-6' : 'p-6 sm:p-8 space-y-6 sm:space-y-8'}`}
                    >
                      <h3 className={`font-serif font-bold text-[var(--charcoal)] tracking-tight flex items-center gap-4 ${isBuyNowMode ? 'text-lg sm:text-xl' : 'text-xl sm:text-2xl'}`}>
                      ...
                      <div className={`grid grid-cols-1 md:grid-cols-2 ${isBuyNowMode ? 'gap-4 pt-4' : 'gap-6 pt-6'}`}>
                        {!(firstItem?.allowGcash === false || firstItem?.allowGcash === 0 || firstItem?.allowGcash === "false" || firstItem?.allowGcash === "0") && (
                          <button
                            type="button"
                            onClick={() => setPaymentMethod("GCash")}
                            className={`border-2 text-left transition-all relative overflow-hidden ${isBuyNowMode ? 'rounded-2xl p-4 sm:p-5' : 'rounded-3xl p-6'} ${paymentMethod === "GCash" ? 'border-blue-500 bg-blue-50/10 shadow-xl ring-4 ring-blue-50' : 'border-[var(--border)] bg-white hover:border-blue-300'}`}
                          >
                            <div className="flex items-start justify-between gap-4 relative z-10">
                              <div className="space-y-3">
                                <div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-700">GCash</div>
                                <div className="font-serif text-base font-bold text-[var(--charcoal)] leading-none sm:text-lg">Pay with GCash</div>
                        ...
                        {!(firstItem?.allowMaya === false || firstItem?.allowMaya === 0 || firstItem?.allowMaya === "false" || firstItem?.allowMaya === "0") && (
                          <button
                            type="button"
                            onClick={() => setPaymentMethod("Maya")}
                            className={`border-2 text-left transition-all relative overflow-hidden ${isBuyNowMode ? 'rounded-2xl p-4 sm:p-5' : 'rounded-3xl p-6'} ${paymentMethod === "Maya" ? 'border-teal-500 bg-teal-50/10 shadow-xl ring-4 ring-teal-50' : 'border-[var(--border)] bg-white hover:border-teal-300'}`}
                          >
                            <div className="flex items-start justify-between gap-4 relative z-10">
                              <div className="space-y-3">
                                <div className="text-[10px] font-black uppercase tracking-[0.2em] text-teal-700">Maya</div>
                                <div className="font-serif text-base font-bold text-[var(--charcoal)] leading-none sm:text-lg">Pay with Maya</div>
                                <p className="text-[11px] font-bold text-[var(--muted)]/60 leading-relaxed uppercase tracking-[0.16em] sm:text-[10px]">Reference & proof of transfer</p>
                              </div>
                          ...
                          <div className={`bg-white border-2 border-[var(--border)] relative overflow-hidden group shadow-lg ${isBuyNowMode ? 'rounded-2xl p-5 sm:p-6 mt-6' : 'rounded-2xl sm:rounded-3xl p-5 sm:p-8 mt-8 sm:mt-10'}`}>
                            ...
                            <div className="flex flex-col items-start gap-5 sm:flex-row sm:items-center sm:gap-8">
                              <div className={`bg-white shadow-2xl overflow-hidden flex items-center justify-center p-2.5 border-2 border-${isGcashPayment ? 'blue' : 'teal'}-100 ring-8 ring-${isGcashPayment ? 'blue' : 'teal'}-50 shrink-0 relative group/qr ${isBuyNowMode ? 'w-24 h-24 rounded-2xl' : 'w-28 h-28 rounded-3xl'}`}>
                                {resolveBackendImageUrl(isGcashPayment ? sellerGcashQrCode : sellerMayaQrCode) ? (
                                  <img src={resolveBackendImageUrl(isGcashPayment ? sellerGcashQrCode : sellerMayaQrCode)} alt="Seller QR Code" className="w-full h-full object-contain rounded-xl" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'block'; }} />
                                ) : null}
                                <Scan className={`w-10 h-10 text-${isGcashPayment ? 'blue' : 'teal'}-100`} style={{ display: resolveBackendImageUrl(isGcashPayment ? sellerGcashQrCode : sellerMayaQrCode) ? 'none' : 'block' }} />
                              </div>
                              ...
                              <div className="flex-1 space-y-4 text-left">
                                <div>
                                  ...
                                  <div className={`font-serif font-black text-${isGcashPayment ? 'blue' : 'teal'}-900 tracking-tight leading-none break-all ${isBuyNowMode ? 'text-lg' : 'text-lg sm:text-xl'}`}>
                            ...
                            <div className="space-y-4">
                              ...
                              <input
                                ...
                                onChange={async (e) => {
                                  ...
                                  if (file) {
                                    const error = await validateImageFile(file, "Payment proof");
                                    if (error) {
                                      alert(error);
                                      e.target.value = "";
                                      setScreenshot(null);
              ...
              <div className={isBuyNowMode ? 'lg:col-span-5 lg:sticky lg:top-20 space-y-6' : 'lg:col-span-4 lg:sticky lg:top-24 space-y-6 lg:space-y-8'}>
                <div className={`artisan-card p-0 overflow-hidden border-2 border-[var(--rust)]/10 bg-white relative ${isBuyNowMode ? 'shadow-2xl' : 'shadow-3xl'}`}>
                  ...
                  <div className={`relative z-10 ${isBuyNowMode ? 'p-6 sm:p-8 space-y-6' : 'p-6 sm:p-8 lg:p-10 space-y-6 sm:space-y-8'}`}>
                    ...
                    <div className={`border-t border-[var(--border)] space-y-4 ${isBuyNowMode ? 'pt-6' : 'pt-8'}`}>
                      ...
                      <div className="pt-2 flex justify-between items-end border-t border-[var(--border)] border-dashed mt-2 pt-6">
                        ...
                        <span className={`font-bold text-[var(--rust)] tracking-tighter ${isBuyNowMode ? 'text-lg' : 'text-xl'}`}>₱<span className="pd-meta-val">{total.toLocaleString()}</span></span>
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={handleNext}
                    disabled={loading || (currentStep !== 3 && maintenanceMode)}
                  ...
                  >
                    ...
                    {loading ? <Loader2 className="w-6 h-6 animate-spin mx-auto" /> : (
                      maintenanceMode && currentStep !== 3 ? <>Orders Paused <ShieldAlert className="w-5 h-5" /></> :
          ...
          {currentStep === 3 && (
            ...
            >
              ...
              <div className="space-y-3 px-6">
                <h2 className="font-serif font-bold text-[var(--charcoal)] tracking-tight text-xl sm:text-2xl uppercase">
                  Order <span className="text-[var(--rust)] italic lowercase">Confirmed</span>
                </h2>
                <p className="text-[var(--muted)] max-w-sm mx-auto leading-relaxed text-xs px-4">
                  The seller has been notified. Your order is now entering the logistics queue for dispatch.
                </p>
              </div>

              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <Link href="/orders" className="btn-primary w-full px-6 py-3.5 shadow-lg flex items-center justify-center gap-2 group sm:w-auto">
                  <span className="text-xs font-bold uppercase tracking-widest">See My Order</span> <History className="w-4 h-4 group-hover:rotate-12 transition-transform" />
                </Link>
                <Link href="/home" className="flex w-full items-center justify-center gap-2 rounded-xl border border-[var(--border)] bg-white px-6 py-3.5 text-[10px] font-bold uppercase tracking-widest shadow-sm transition-all hover:border-[var(--rust)] hover:text-[var(--rust)] sm:w-auto">
                  Back to Shop <ShoppingCart className="w-4 h-4" />
                </Link>
              </div>
          ...
          {showValidation && (
            ...
            >
              <motion.div
                ...
                className="artisan-card relative z-20 w-full max-w-md border-red-100 p-6 text-center shadow-[0_40px_100px_rgba(0,0,0,0.5)] space-y-6 sm:p-8"
              >
                ...
                <div className="space-y-4">
                  <h3 className="font-serif text-2xl font-bold text-[var(--charcoal)] tracking-tight sm:text-3xl">Registry <span className="text-red-600">Errors</span></h3>
                ...
                <div className="inline-block w-full space-y-3 rounded-2xl border border-red-100/50 bg-red-50/50 p-5 text-left sm:rounded-3xl sm:p-6">
                  {currentStep === 1 ? (
                    <>
                ...
                <button
                  onClick={() => setShowValidation(false)}
                  className="w-full rounded-2xl bg-red-600 py-4 text-xs font-extrabold uppercase tracking-[0.18em] text-white shadow-2xl transition-all hover:bg-red-700 active:scale-95 sm:py-5 sm:text-[10px] sm:tracking-widest"
                >
