  ...
  try {
    ...
    const totalCustomersCount = await User.count({ 
      where: { 
        ...where, 
        role: 'customer',
        status: { [Op.ne]: 'blocked' } // Exclude terminated users for dashboard consistency
      } 
    });
...
exports.deleteCustomer = async (req, res) => {
  try {
    ...
    console.log(`Terminating user ${user.email}. Reason: ${normalizedReason || 'No reason provided.'}`);
    ...
    user.status = 'blocked';
    user.violationReason = normalizedReason || 'Account terminated by administrator.';
    await user.save();
...
exports.toggleCustomerStatus = async (req, res) => {
  try {
    ...
    // without waiting out the remaining lock period from earlier spam attempts
    resetLoginRateLimit(user.email);

    emitUserUpdated(user, { action: 'status_changed' });

    res.status(200).json({ message: `User unfrozen successfully`, user });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
exports.freezeUser = async (req, res) => {
  try {
    ...
    user.status = 'frozen';
    user.violationReason = normalizedReason;
    await user.save();
...
exports.rejectSeller = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Seller application rejected with reason' });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};

const cleanSettingValue = (val) => {
...
exports.getPublicSettings = async (req, res) => {
  try {
    const keys = ['landingPageBackground', 'landingPageBackgroundPosition', 'maintenanceMode'];
    const settings = await SystemSetting.findAll({
