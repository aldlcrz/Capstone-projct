...
exports.getProfile = async (req, res) => {
    try {
        ...
        if (userData.addresses && userData.addresses.length > 0) {
            ...
            // Legacy sync fallback: if user has no mobileNumber but has a default address, use the address phone
            if (!userData.mobileNumber && defaultAddr.phone) {
...
exports.createAddress = async (req, res) => {
    try {
        ...
        if (isDefault) {
            ...
            await User.update({ mobileNumber: phone }, { where: { id: req.user.id } });
        }
...
exports.setDefaultAddress = async (req, res) => {
    try {
        ...
        await User.update({ mobileNumber: address.phone }, { where: { id: req.user.id } });
...
exports.getSellerInfo = async (req, res) => {
    try {
        ...
        const seller = await User.findByPk(id, {
            attributes: ['id', 'name', 'createdAt', 'role', 'facebookLink', 'instagramLink', 'tiktokLink', 'youtubeLink', 'socialLinks', 'profilePhoto', 'isVerified', 'shopCity', 'shopProvince', 'indigencyCertificate', 'shopLatitude', 'shopLongitude']
        });
        ...
        const avgRating = parseFloat(reviewData[0]?.avgRating || 0).toFixed(1);
