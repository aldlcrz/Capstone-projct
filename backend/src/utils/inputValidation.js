...
const validatePaymentReference = (value, fieldName = 'Payment reference number') => {
  ...
  const refRegex = new RegExp(`^[a-zA-Z0-9]{${LIMITS.paymentReferenceMin},${LIMITS.paymentReferenceMax}}$`);

  if (!refRegex.test(normalized)) {
    throw createValidationError(
      `${fieldName} must be alphanumeric and between ${LIMITS.paymentReferenceMin} and ${LIMITS.paymentReferenceMax} characters`
    );
  }
...
const validateAddressPayload = (payload = {}) => {
  ...
  return {
    ...
    region: validatePlaceName(payload.region, 'Region', LIMITS.region),
