<aside class="w-full md:w-56 shrink-0">
    {{-- User identity --}}
    <div class="flex items-center gap-3 pb-4 md:pb-5 border-b border-gray-100 mb-4 px-1">
        <div class="w-10 h-10 md:w-12 md:h-12 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center overflow-hidden shrink-0">
            @if($user->profile_photo_url)
                <img src="{{ $user->profile_photo_url }}" class="w-full h-full object-cover">
            @else
                <span class="text-base md:text-lg font-bold text-gray-400">{{ strtoupper(substr($user->name, 0, 1)) }}</span>
            @endif
        </div>
        <div class="min-w-0">
    ...
    {{-- Mobile Navigation Tabs (< md) --}}
    <div class="md:hidden flex items-center gap-2 overflow-x-auto no-scrollbar pb-2 mb-4">
        <a href="{{ route('profile') }}"
           class="px-4 py-2 text-xs font-bold rounded-full shrink-0 transition-colors
           {{ request()->routeIs('profile') && !request()->routeIs('profile.addresses') && !request()->routeIs('profile.change-password')
              ? 'bg-[#C0420A] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
            Profile
        </a>
        <a href="{{ route('profile.addresses') }}"
           class="px-4 py-2 text-xs font-bold rounded-full shrink-0 transition-colors
           {{ request()->routeIs('profile.addresses') ? 'bg-[#C0420A] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
            Addresses
        </a>
        <a href="{{ route('profile.change-password') }}"
           class="px-4 py-2 text-xs font-bold rounded-full shrink-0 transition-colors
           {{ request()->routeIs('profile.change-password') ? 'bg-[#C0420A] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
            Change Password
        </a>
        <a href="{{ route('orders') }}"
           class="px-4 py-2 text-xs font-bold rounded-full shrink-0 transition-colors bg-gray-100 text-gray-600 hover:bg-gray-200 flex items-center gap-1.5">
            <svg class="w-3.5 h-3.5 text-[#C0420A]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
            My Purchase
        </a>
    </div>
