...
<div class="space-y-6 sm:space-y-8 max-w-6xl pb-28 lg:pb-12 px-2 sm:px-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6">
        ...
        <div class="rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-xs space-y-1 sm:space-y-2" style="background: #FFFCF7; border: 1px solid #E8DECB;">
            ...
            <div class="text-xl sm:text-2xl font-black font-sans text-[#C49520]">₱{{ number_format($totalSales, 2) }}</div>
            <div class="text-xs text-[#766C60]">Total completed &amp; active orders</div>
        </div>

        <div class="text-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-xl space-y-1 sm:space-y-2 relative overflow-hidden" style="background: #1E1915; border: 1px solid #C49520;">
            <div class="text-[10px] font-bold uppercase tracking-widest text-amber-200/70">Net Commission Due</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-8">
        ...
        <div class="rounded-2xl sm:rounded-3xl p-5 sm:p-8 shadow-xs space-y-4 sm:space-y-6" style="background: #FFFCF7; border: 1px solid #E8DECB;">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {{-- GCash --}}
                <div class="p-4 rounded-2xl space-y-3 text-center" style="background: #FDF8EE; border: 1px solid #E8DECB;">
                    ...
                    @if($paymentSettings['gcash_qr'])
                        <img src="{{ str_starts_with($paymentSettings['gcash_qr'], 'http') || str_starts_with($paymentSettings['gcash_qr'], '/') ? $paymentSettings['gcash_qr'] : asset('storage/' . $paymentSettings['gcash_qr']) }}" class="w-36 h-36 object-contain mx-auto rounded-xl border border-white shadow-xs" onerror="this.style.display='none'">
                    @else
                ...
                <div class="p-4 rounded-2xl space-y-3 text-center" style="background: #FDF8EE; border: 1px solid #E8DECB;">
                    ...
                    @if($paymentSettings['maya_qr'])
                        <img src="{{ str_starts_with($paymentSettings['maya_qr'], 'http') || str_starts_with($paymentSettings['maya_qr'], '/') ? $paymentSettings['maya_qr'] : asset('storage/' . $paymentSettings['maya_qr']) }}" class="w-36 h-36 object-contain mx-auto rounded-xl border border-white shadow-xs" onerror="this.style.display='none'">
                    @else
                        <div class="h-36 bg-white/60 rounded-xl flex items-center justify-center text-[10px] text-gray-400 italic">No QR uploaded</div>
                    @endif
                </div>
            ...
            @else
                <form action="{{ route('seller.commission.submit') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="referenceNumber" required placeholder="e.g. 10029384812" class="w-full h-11 px-4 rounded-xl text-xs font-semibold outline-none font-sans" style="background: #FDF8EE; border: 1px solid #E8DECB; color: #1E1915;">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="2" placeholder="Additional details..." class="w-full p-3 rounded-xl text-xs outline-none resize-none" style="background: #FDF8EE; border: 1px solid #E8DECB; color: #1E1915;"></textarea>
                    </div>

                    <button type="submit" class="w-full h-12 text-white rounded-xl text-xs font-bold uppercase tracking-widest transition-all shadow-xs cursor-pointer" style="background: #1E1915;" onmouseover="this.style.background='#C49520';" onmouseout="this.style.background='#1E1915';">
    ...
    <div class="rounded-2xl sm:rounded-3xl shadow-xs overflow-hidden space-y-4" style="background: #FFFCF7; border: 1px solid #E8DECB;">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left min-w-125">
                <thead>
                    <tr class="border-b" style="background: #FDF8EE; border-color: #E8DECB;">
                        <th class="px-4 sm:px-6 py-3.5 text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-[#766C60]">Period</th>
                        <th class="px-4 sm:px-6 py-3.5 text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-[#766C60]">Sales</th>
                ...
                <tbody class="divide-y text-xs font-sans" style="border-color: #F0EAE1; color: #1E1915;">
