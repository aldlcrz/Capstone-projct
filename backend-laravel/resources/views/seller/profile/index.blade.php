...
@section('content')
    ...
    <div class="min-h-[calc(100vh-120px)] px-3 py-4 sm:px-6 sm:py-8 pb-28 lg:pb-12" 
         ...
         x-init="
             if (new URLSearchParams(window.location.search).get('open_payment') === '1' || window.location.hash === '#payment-methods') {
    ...
    {{-- =====================================================
         DOWNLOAD YOUR INFORMATION MODAL
         ===================================================== --}}
    <div x-show="showDownloadInfoModal"
         x-cloak
        ...
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm"
             @click="showDownloadInfoModal = false"></div>

        {{-- Modal Card --}}
        <div class="relative w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden"
             style="background:#FDFBF7;border:1px solid #EAE2D2;">

            {{-- Header --}}
            <div class="px-6 pt-6 pb-4 border-b border-[#EAE2D2] flex items-center gap-3">
                <div style="width:40px;height:40px;border-radius:12px;background-color:#FAF5EA;border:1px solid #E6D8BA;display:flex;align-items:center;justify-content:center;color:#B88728;flex-shrink:0;">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                    </svg>
                </div>
                <div>
                    <h3 class="font-serif text-base font-bold text-[#1E1915] m-0">Download Your Information</h3>
                    <p class="text-xs text-[#78716C] m-0">Export a copy of your shop data</p>
                </div>
                <button type="button" @click="showDownloadInfoModal = false"
                        class="ml-auto text-[#8C827A] hover:text-[#1E1915] cursor-pointer transition-colors p-1 rounded-lg hover:bg-[#F0EAE0]">
            ...
            <div class="px-6 py-5 space-y-3">
                <p class="text-xs text-[#4A4038]">Your export will include the following data as a <strong>ZIP archive</strong>:</p>
                <ul class="space-y-2 text-xs text-[#5C5249]">
                    <li class="flex items-start gap-2">
                        <svg class="w-3.5 h-3.5 text-[#C49520] mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                        Shop profile &amp; account details
                    </li>
                    <li class="flex items-start gap-2">
                        <svg class="w-3.5 h-3.5 text-[#C49520] mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                        All your listed products &amp; inventory
                    </li>
                    <li class="flex items-start gap-2">
                        <svg class="w-3.5 h-3.5 text-[#C49520] mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                ...
                <p class="text-[10.5px] text-[#8C827A] pt-1">Preparing the archive may take a moment. The file will download automatically.</p>
            </div>
            ...
            <div class="px-6 pb-6 flex gap-3">
                <button type="button"
                        @click="showDownloadInfoModal = false"
                        class="flex-1 py-2.5 rounded-xl border border-[#E2D9C8] text-[#78716C] text-xs font-bold hover:bg-[#F0EAE0] transition-colors cursor-pointer">
                    Cancel
                </button>
                <a href="{{ route('profile.download-information') }}"
                   @click="showDownloadInfoModal = false"
                   class="flex-1 py-2.5 rounded-xl text-white text-xs font-bold text-center flex items-center justify-center gap-1.5 transition-all shadow-md"
                   style="background:#1E1915;">
    ...
    <div x-show="showDeleteAccountModal"
        ...
        <div class="absolute inset-0 bg-black/50 backdrop-blur-sm"
             @click="showDeleteAccountModal = false; deleteAccountConfirmation = ''"></div>

        {{-- Modal Card --}}
        <div class="relative w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden"
             style="background:#FDFBF7;border:1px solid #FED7D7;">

            {{-- Header --}}
            <div class="px-6 pt-6 pb-4 border-b border-red-100 flex items-center gap-3">
                <div style="width:40px;height:40px;border-radius:12px;background-color:#FED7D7;border:1px solid #FEB2B2;display:flex;align-items:center;justify-content:center;color:#9B2C2C;flex-shrink:0;">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                    </svg>
                </div>
                <div>
                    <h3 class="font-serif text-base font-bold text-red-800 m-0">Delete Account</h3>
                    <p class="text-xs text-red-400 m-0">This schedules a 7-day recovery period</p>
                </div>
                <button type="button"
                        @click="showDeleteAccountModal = false; deleteAccountConfirmation = ''"
                        class="ml-auto text-[#8C827A] hover:text-[#1E1915] cursor-pointer transition-colors p-1 rounded-lg hover:bg-red-50">
                    <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                </button>
            ...
            <div class="px-6 py-5 space-y-4">
                <div class="bg-red-50 border border-red-100 rounded-xl p-3 text-xs text-red-700 space-y-1.5">
                    <p class="font-bold">⚠️ Before you proceed:</p>
                    <ul class="space-y-1 pl-3 list-disc">
                        <li>Your shop will be <strong>hidden immediately</strong> from buyers.</li>
                        <li>You have <strong>7 days</strong> to log in and cancel this request.</li>
                        <li>After 7 days, your account and all data will be <strong>permanently deleted</strong>.</li>
                        <li>Active orders should be completed before deleting.</li>
                    </ul>
                </div>

                <div>
                    <label class="block text-xs font-bold text-[#4A4038] mb-1.5">
                        Type <span class="font-black text-red-700 tracking-wider">DELETE</span> to confirm:
                    </label>
                    <input type="text"
                           x-model="deleteAccountConfirmation"
                           placeholder="DELETE"
                           class="w-full px-3 py-2.5 rounded-xl border text-sm font-mono tracking-widest text-center focus:outline-none transition-colors"
                           :class="deleteAccountConfirmation === 'DELETE'
                               ? 'border-red-400 bg-red-50 text-red-700 focus:ring-2 focus:ring-red-300'
                               : 'border-[#E2D9C8] bg-white text-[#1E1915] focus:border-[#C49520] focus:ring-2 focus:ring-[#C49520]/20'">
                </div>
            </div>
            ...
            <div class="px-6 pb-6 flex gap-3">
                ...
                <form action="{{ route('profile.delete-account') }}" method="POST" class="flex-1">
                    @csrf
                    <button type="submit"
