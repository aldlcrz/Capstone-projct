...
@section('content')
<div class="greeting">Mabuhay, {{ $customerName ?? ($userName ?? 'Valued Customer') }}! 🏷️</div>
...
<div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px; margin: 16px 0;">
    ...
    <div style="margin-top: 6px;">
        <span style="text-decoration: line-through; color: #94a3b8; font-size: 13px;">₱{{ number_format($originalPrice ?? ($oldPrice ?? 0), 2) }}</span>
        <span style="font-weight: 800; font-size: 16px; color: #C0420A; margin-left: 8px;">₱{{ number_format($salePrice ?? ($newPrice ?? 0), 2) }}</span>
