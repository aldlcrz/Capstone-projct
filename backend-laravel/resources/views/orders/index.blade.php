...
<div class="max-w-[1000px] mx-auto py-8">
    <div class="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
        ...
        <form action="/orders/my-orders" method="GET" class="relative w-full md:w-64">
    ...
    <div class="flex border-b border-gray-100 mb-8 overflow-x-auto no-scrollbar gap-2">
        @foreach(['ALL', 'PENDING', 'TO SHIP', 'TO RECEIVE', 'COMPLETED'] as $tab)
            <a href="/orders/my-orders?tab={{ $tab }}" class="px-6 py-4 text-[10px] font-bold uppercase tracking-widest transition-all border-b-2 {{ (request('tab', 'ALL') == $tab) ? 'border-[#C0422A] text-[#C0422A]' : 'border-transparent text-gray-400 hover:text-black' }}">
                {{ $tab }}
            </a>
        @endforeach
    </div>
