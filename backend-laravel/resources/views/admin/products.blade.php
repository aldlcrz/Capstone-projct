    ...
    <div class="space-y-8" x-data="{ rejectModal: false, rejectProductId: null }">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <span
                    class="px-3 py-1 bg-orange-100 text-orange-600 rounded-full text-[10px] font-bold uppercase tracking-widest">
        ...
        <div x-show="rejectModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
            ...
            <div class="relative bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl">
                ...
                <form :action="'/admin/products/reject/' + rejectProductId" method="POST">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 block">Reason
                                for Rejection</label>
                            <textarea name="reason" rows="4"
                                class="w-full bg-gray-50 border border-gray-100 rounded-2xl p-4 text-sm focus:outline-none focus:border-black"
                                placeholder="Explain why the product was rejected..." required></textarea>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <button type="button" @click="rejectModal = false"
                                class="py-3 bg-gray-50 text-gray-400 text-[10px] font-bold uppercase tracking-widest rounded-xl">Cancel</button>
                            <button type="submit"
                                class="py-3 bg-red-500 text-white text-[10px] font-bold uppercase tracking-widest rounded-xl shadow-lg shadow-red-500/20">Confirm
                                Rejection</button>
                        </div>
                    </div>
                </form>
