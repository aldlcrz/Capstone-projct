...
<div class="space-y-8" x-data="{ 
                ...
                <tr class="group hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <button @click="resolvingReport = { 
                                id: '{{ $report->id }}', 
                                reason: '{{ addslashes($report->reason) }}', 
                                description: '{{ addslashes($report->description) }}',
                                evidence: '{{ $report->evidence }}',
                                reporterName: '{{ addslashes($report->reporter->name ?? 'Deleted User') }}',
    ...
    <div x-show="showResolveModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" x-cloak>
        <div class="bg-white w-full max-w-3xl rounded-[2.5rem] p-8 shadow-2xl overflow-y-auto max-h-[90vh]" @click.away="showResolveModal = false">
            <h3 class="text-xl font-black text-[#3D2B1F] mb-6 uppercase tracking-tight">Report Details & Resolution</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <!-- Left: Details -->
                <div class="space-y-4">
                    <div class="bg-stone-50 border border-stone-100 p-5 rounded-2xl space-y-4">
                        <div>
                            <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block">Reporter</span>
                            <div class="flex items-center gap-1.5 mt-0.5">
                                <span class="text-xs font-bold text-[#3D2B1F]" x-text="resolvingReport.reporterName"></span>
                                <span class="text-[8px] px-1.5 py-0.5 bg-stone-200 text-stone-700 rounded font-black uppercase tracking-wider" x-text="resolvingReport.reporterRole"></span>
                            </div>
                        </div>
                        <div>
                            <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block">Reported Account</span>
                            <span class="text-xs font-bold text-[#3D2B1F] mt-0.5 block" x-text="resolvingReport.reportedName"></span>
                        </div>
                        <div>
                            <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block">Submitted On</span>
                            <span class="text-xs font-bold text-gray-600 mt-0.5 block" x-text="resolvingReport.createdAt"></span>
                        </div>
                        <div class="border-t border-stone-200/60 pt-3">
                            <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block">Reason</span>
                            <span class="text-xs font-black text-red-600 uppercase tracking-wide mt-0.5 block" x-text="resolvingReport.reason"></span>
                        </div>
                        <div>
                            <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block">Detailed Description</span>
                            <p class="text-[11px] text-gray-600 leading-relaxed italic mt-1 bg-white border border-stone-100 rounded-xl p-3" x-text="'“' + resolvingReport.description + '”'"></p>
                        </div>
                    </div>

                    <!-- Evidence Image -->
                    <template x-if="resolvingReport.evidence">
                        <div class="border border-stone-100 rounded-2xl overflow-hidden bg-stone-50 p-4">
                            <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-2">Evidence Attachment</span>
                            <a :href="resolvingReport.evidence" target="_blank" class="block group relative aspect-video w-full rounded-xl overflow-hidden border border-stone-200 bg-white">
                                <img :src="resolvingReport.evidence" class="w-full h-full object-cover">
                ...
                <!-- Right: Action Form -->
                <div>
                    <form :action="'/admin/reports/' + resolvingReport.id + '/resolve'" method="POST" class="space-y-5">
                        @csrf
                        @method('PATCH')
                        <div>
                            <label class="text-[10px] font-black text-[#3D2B1F] uppercase tracking-widest block mb-2">Action Taken</label>
                            <select name="action" required class="w-full px-4 py-3.5 bg-stone-50 border border-stone-150 rounded-xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-rust/20">
                        ...
                        <div>
                            <label class="text-[10px] font-black text-[#3D2B1F] uppercase tracking-widest block mb-2">Resolution Notes</label>
                            <textarea name="notes" rows="6" required placeholder="Explain the outcome of this investigation..." class="w-full px-4 py-3.5 bg-stone-50 border border-stone-150 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-rust/20"></textarea>
                        </div>
                        <div class="flex gap-3 pt-4">
                            <button type="button" @click="showResolveModal = false" class="flex-1 px-5 py-4 bg-gray-50 text-gray-700 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-150 transition-all">Cancel</button>
                            <button type="submit" class="flex-1 px-5 py-4 bg-[#3D2B1F] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all">Confirm Resolution</button>
                        </div>
                    </form>
                </div>
            </div>
