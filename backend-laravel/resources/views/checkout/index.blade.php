...
function checkoutApp(initialAddress, initialAddresses, defaultPaymentMethod) {
    return {
        ...
        requestPlaceOrder() {
            if (this.aiChecking) {
                this.screenshotError = 'Please wait while the receipt image scanning is in progress.';
                return;
            }
            ...
            if (this.aiVerificationResult && (this.aiVerificationResult.is_receipt === false || this.aiVerificationResult.status === 'REJECT')) {
                this.screenshotError = this.aiVerificationResult.message || 'Attached file is not a valid receipt. Please attach a genuine transaction receipt.';
                document.getElementById('paymentScreenshotInput')?.focus();
                return;
            }
            this.screenshotError = '';
