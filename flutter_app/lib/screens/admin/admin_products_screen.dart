...
class _AdminProductsScreenState extends State<AdminProductsScreen>
  ...
  Future<void> _addCategory() async {
    ...
    try {
      await ApiClient().post('/categories', data: {
        'name': name,
        'description': _catDescController.text.trim(),
      });
      if (mounted) {
  ...
  Future<bool?> _showConfirmDialog(String title, String content) {
    return showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text(title, style: const TextStyle(fontWeight: FontWeight.w900, color: AppTheme.charcoal)),
        content: Text(content, style: const TextStyle(color: AppTheme.textMuted, fontSize: 13)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('CANCEL', style: TextStyle(fontWeight: FontWeight.w800, color: AppTheme.textMuted)),
          ),
          ElevatedButton(
            ...
            child: const Text('DELETE', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 12)),
          ),
        ],
      ),
    );
  ...
  void _showError(String msg) {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(msg), behavior: SnackBarBehavior.floating, backgroundColor: Colors.red),
    );
  }
              ...
              children: [
                ...
                Container(
                  ...
                  child: TabBar(
                    ...
                    tabs: const [
                      Tab(text: 'PRODUCTS', icon: Icon(Icons.inventory_2_outlined, size: 16)),
                      Tab(text: 'CATEGORIES', icon: Icon(Icons.grid_view_outlined, size: 16)),
                    ],
                  ),
                ),
              ],
      ...
      children: [
        ...
        Expanded(
          child: _filteredProducts.isEmpty
              ? _buildEmptyState('No products found', 'Try adjusting your search filters.')
              : RefreshIndicator(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      if (rawImages is List && rawImages.isNotEmpty) {
                        ...
                        if (first is String) {
                          ...
                          thumbnailUrl = first['url']?.toString() ?? first['path']?.toString();
                        }
                            ...
                            Container(
                              ...
                              child: thumbnailUrl != null
                                  ? CachedNetworkImage(
                                      ...
                                      fit: BoxFit.cover,
                                      errorWidget: (context, url, error) => const Icon(Icons.inventory_2, color: AppTheme.borderLight),
                                    )
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      Icon(Icons.storefront, size: 10, color: AppTheme.primary),
                                      const SizedBox(width: 4),
                                      Expanded(
                                        child: Text(
                                          (seller['shopName'] ?? seller['name'] ?? 'Unknown Artisan').toString(),
                                          style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: AppTheme.charcoal),
                                          maxLines: 1,
                                          overflow: TextOverflow.ellipsis,
                                        ),
                                      ),
      ...
      child: Column(
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                Text(
                  ...
                  style: GoogleFonts.playfairDisplay(fontSize: 18, fontWeight: FontWeight.w800, color: AppTheme.charcoal),
                ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: _isAddingCategory 
                      ...
                      : const Text('ADD TO REGISTRY', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 2)),
                  ),
                ),
              ],
            ),
          ...
          if (_categories.isEmpty)
            _buildEmptyState('No segments registered', 'Add a category above to get started.')
          else
            ListView.separated(
              shrinkWrap: true,
                ...
                return Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      IconButton(
                        onPressed: () => _deleteCategory(c['id'].toString(), c['name'].toString()),
                        icon: Icon(Icons.delete_outline, color: Colors.red.shade400, size: 20),
                        style: IconButton.styleFrom(
  ...
  Widget _buildEmptyState(String title, String subtitle) {
    return Center(
      child: Padding(
        ...
        child: Column(
          children: [
            Icon(Icons.inbox_outlined, size: 48, color: AppTheme.textMuted.withValues(alpha: 0.2)),
            const SizedBox(height: 16),
            Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: AppTheme.charcoal)),
            const SizedBox(height: 4),
